...
var phaseList = {
  Pregame: [
    ...
    "Declare Reserves and Transports",
    "before the battle",
  ],
  Command: [],
...
function openTab(title, all) {
  for (var header of document.getElementsByClassName("accordion-header")) {
    ...
    if (header.classList.contains("isOpen"))
      if (all) toggleAccordion(header);
      else if (header.parentNode.id == "content") toggleAccordion(header);
    if (window.innerWidth <= 635) {
      closeNav(toggleatkr);
      closeNav(toggledfdr);
    }
    // If it's the chosen header, open it back up
    if (header.innerText == title) toggleAccordion(header);
  }
}
...
function toggleNav(el) {
  ...
  if (
    target.classList.contains("isActive") ||
    target.classList.contains("isOpen")
  ) {
...
function closeNav(el) {
  if (el.id == "toggleatkr") target = document.getElementById("atkrList");
  else target = document.getElementById("dfdrList");
  target.classList.remove("isOpen");
  target.classList.remove("isActive");
  el.innerHTML = "&#9776;";
}
...
function getSiblings(e) {
  ...
  do {
    if (
      ...
      (e.classList.contains("accordion-header") ||
        e.classList.contains("accordion-content"))
    ) {
      siblings.push(e);
    }
...
function weaponSelect() {
  for (var weaponElement of document.getElementsByClassName("weapon")) {
    ...
    weaponElement.addEventListener("click", (e) => {
      if (weapon.user.customName)
        ioList.attackerName.innerHTML = weapon.user.customName + "<br>";
      else ioList.attackerName.innerHTML = weapon.user.name + "<br>";
      if (weapon.customName)
        ioList.attackerName.innerHTML += weapon.customName + "<br>";
...
function setTrackers(emit) {
  ...
  for (tracker_player of document.getElementsByClassName("vpbox")) {
    ...
    for (tracker of tracker_player.getElementsByClassName("vptracker")) {
      ttltracker.innerText =
        Number(ttltracker.innerText) + Number(tracker.value);
      if (!score[tracker.id.split("_")[0]]) {
        score[tracker.id.split("_")[0]] = {};
      }
      score[tracker.id.split("_")[0]][tracker.id.split("_")[1]] = Number(
        tracker.value
      );
    }
    score[tracker_player.id.split("_")[0]].ttl = Number(ttltracker.innerText);
    score[tracker_player.id.split("_")[0]].cp = Number(
      document.getElementById(tracker_player.id.split("_")[0] + "_cp").value
    );
  }
  if (emit) {
    if (gameCode) {
...
function getTrackers(score) {
  for (tracker_player of document.getElementsByClassName("vpbox")) {
    document.getElementById(tracker_player.id.split("_")[0] + "_cp").value =
      Number(score[tracker_player.id.split("_")[0]].cp);
    for (tracker of tracker_player.getElementsByClassName("vptracker")) {
      tracker.value = Number(
        score[tracker.id.split("_")[0]][tracker.id.split("_")[1]]
      );
    }
  }
  setTrackers();
...
function updatePhaseTags() {
  for (nestedPhase of document
    .getElementById("phaser")
    .getElementsByClassName("phaseTag")) {
...
function buildSecondaries(secondaries, listIndex, edit) {
  if (edit && secondaries.length) {
    ...
    let secondList = {};
    //find all unique
    for (var secondCat of secondaries
      .map((item) => item.category)
      .filter((value, index, self) => self.indexOf(value) === index)) {
      // Sort by category
      for (var second of secondaries) {
        if (secondCat == second.category) {
          if (secondList[secondCat]) secondList[secondCat].push(second);
    ...
    for (var secondCat of Object.keys(secondList)) {
      ...
      for (var second of secondList[secondCat]) {
        ...
        for (let i = 0; i < secondParse.length; i++) {
          ...
          secondName +=
            secondParse[i][0].toUpperCase() + secondParse[i].substr(1) + " ";
        }
...
function changeKolor(stylesheetName) {
  document
    .getElementById("40kolor")
    .setAttribute("href", "css/40kolor_" + stylesheetName + ".css");
}
...
function tickle() {
  ...
  if (tickles == 10) {
    ...
    document.getElementById("subbanner").innerText =
      "ウォーハンマー40k試合電卓";
  }
}
...
function customMarkdown(notesString) {
  ...
  do {
    ...
    if (checkMatch != null) {
      let fileName = "";
      if (checkMatch[1] == "-") fileName = "null";
      else fileName = checkMatch[1];
      notesString = notesString.replaceAll(
        `{${checkMatch[1]}}`,
        `<img src="img/icons/cp${fileName}.svg" style="margin: 0px; padding: 0px; width: 12px; height: 12px;">`
      );
    }
  } while (checkMatch != null);
  // Bold
  ...
  do {
    ...
    if (checkMatch != null)
      if (checkMatch[1].length <= 5)
        notesString = notesString.replaceAll(
          `${checkMatch[1]} ${checkMatch[2]}${checkMatch[3]}`,
          `<h${checkMatch[1].length}>${checkMatch[2]}</h${checkMatch[1].length}>`
        );
      else
        notesString = notesString.replaceAll(
          `${checkMatch[1]} ${checkMatch[2]}`,
  ...
  do {
    ...
    if (checkMatch != null) {
      notesString = notesString.replaceAll(
        `![${checkMatch[1]}](${checkMatch[2]})`,
        `<img src="${checkMatch[2]}" alt="${checkMatch[1]}">`
      );
    }
  } while (checkMatch != null);
  // link
  ...
  do {
    ...
    if (checkMatch != null) {
      ...
      notesString = notesString.replaceAll(
        `[${checkMatch[1]}](${checkMatch[2]})`,
        `<a href="${checkMatch[2]}">${checkMatch[1]}</a>`
      );
    }
  } while (checkMatch != null);
  // Dice emojis
...
function init(mode) {
  ...
  if (forceData.length == 2) {
    if (forceData[0].force.gametype != forceData[1].force.gametype) {
      output(
        `<span class="oR" title="This is non-fatal. You can still use the app, but probably with limited functionality.">Warning: </span>Forces' game types do not match!`
      );
    }
  }
...
function listBuild() {
  ...
  for (var force of forceData) {
    if (force.force) {
      ...
      if (force.force.customNotes)
        thisList.innerHTML += `<h3 class='textSans'>${customMarkdown(
          force.force.customNotes
        )}</p>`;
      ...
      */
      ...
      for (var detachment of force.force.detachments) {
        if (settings.forceSort == "slot")
          detachment.units = sortBySlot(detachment.units);
        else if (["pts", "pl", "cp"].includes(settings.forceSort))
          detachment.units = sortByCost(detachment.units, settings.forceSort);
        else if (settings.forceSort)
          detachment.units = sortByProperty(
            detachment.units,
            settings.forceSort
          );
        ...
        if (detachment.factionIcon) appendList.src = detachment.factionIcon;
        // If it couldn't load the image
        newBox.append(appendList);
        // Add detachment name
        appendList = document.createElement("h3");
        appendList.innerText = detachment.customName
          ? detachment.customName
          : detachment.name;
        newBox.append(appendList);
        // Add detachment faction
        appendList = document.createElement("h3");
        if (detachment.factionLink)
          appendList.innerHTML += `<a href="${detachment.factionLink}" target="_blank">${detachment.faction}</a>`;
        ...
        for (var unit of detachment.units) {
          if (!settings.slotIcons && settings.forceSort == "slot") {
            ...
            if (detachment.units.indexOf(unit)) {
              if (
                detachment.units[detachment.units.indexOf(unit) - 1].slot !=
                unit.slot
              )
          ...
          if (unit.customNotes) {
            ...
            if (headerColor) {
              unit.customNotes = unit.customNotes.replace(
          ...
          if (
            settings.slotIcons &&
            unitRoles.includes(unit.slot.replaceAll(" ", "").toLowerCase())
          ) {
          ...
          for (var model of unit.models) {
            ...
            */
            for (var weapon of model.weapons) {
              ...
              nameDiv.innerHTML += weapon.customName
                ? weapon.customName
                : weapon.name;
              if (weapon.amount) nameDiv.innerHTML += ` x ${weapon.amount}`;
              weaponDiv.append(nameDiv);
              if (weapon.customNotes)
                weaponDiv.innerHTML += `<p class='textSmall textSans'>${customMarkdown(
                  weapon.customNotes
                )}</p>`;
              for (var stat of Object.keys(weapon)) {
                if (stat == "abilities") {
                  ...
                  else statLbl.innerHTML += stat.toUpperCase();
              ...
              if (weapon.abilities && weapon.abilities != "-") {
                ...
                if (settings.statIcons) {
                  ...
                  if (
                    ...
                    abilText = abilText
                      .replaceAll(
                        /Each time an attack is made with this weapon, that attack automatically hits the target[.]?/g,
                        ""
                      )
                      .trim();
                  ...
                  if (
                    ...
                    let multi =
                      /Each time an attack is made with this weapon profile, make (\d)/g.exec(
                        abilText
                      );
                    abilText = abilText
                      .replaceAll(
                        /Each time an attack is made with this weapon profile, make (\d) hit rolls instead of 1./g,
                        ""
                      )
                      .trim();
                    if (multi)
          ...
          */
          ...
          for (var spell of unit.spells) {
            ...
            if (spell.customNotes)
              newSpellContent.innerHTML += `<p class='textSmall textSans'>${customMarkdown(
                spell.customNotes
              )}</p>`;
          ...
          for (var rule of unit.rules) {
            ...
            if (rule.name == "Explodes") {
              ...
              newRuleContent.innerHTML += `
							<div class="textSmall cLeft textSans">${rule.desc.replaceAll(
                currentPhase + " phase",
                `<span class="oR" title="${currentPhase}">${currentPhase} phase</span>`
              )}</div>
							<div class="statRow noBorder">
          ...
          */
          if (settings.stratagems) {
            ...
            for (var stratagem of unit.usestratagems) {
              ...
              if (
                ...
                (detachment.subfaction.length == 0 ||
                  detachment.subfaction == stratagem.subfaction ||
                  detachment.faction == stratagem.subfaction ||
                  detachment.variant == stratagem.subfaction)
              ) {
                let newStratagem = document.createElement("div");
                if (detachment.faction != stratagem.subfaction)
                  if (detachment.subfaction)
                    newStratagem.setAttribute(
                      `data-subfaction`,
                      detachment.subfaction
                    );
                newStratagem.classList.add(
                  "accordion-header",
                  "tag",
                  "mini",
                  "stratTag",
                ...
                let fileName = "";
                if ("0123456789".includes(stratagem.cp_cost))
                  fileName = stratagem.cp_cost;
                else fileName = "null";
                ...
                for (let i = 0; i < stratParse.length; i++) {
                  ...
                  stratName +=
                    stratParse[i][0].toUpperCase() +
                    stratParse[i].substr(1) +
                    " ";
                }
                ...
                newStratagemContent.classList.add(
                  "accordion-content",
                  "hide",
                  "bg6",
                  "hlSome"
                ...
                newStratagemContent.innerHTML += `
		            				...
		            				<p class="textSmall cLeft textSans">${stratagem.description}</p>
		          					`;
                for (phase of Object.keys(phaseList)) {
                  if (makeUseTag(stratagem.description, phase))
                    newStratagem.classList.add(phase);
                }
          ...
          picSearch.setAttribute(
            "data-link",
            `https://www.google.com/search?tbm=isch&q=Warhammer%2040000%20${unit.name}%20miniature`
          );
          ...
          fandomSearch.setAttribute(
            "data-link",
            `https://warhammer40k.fandom.com/wiki/Special:Search?scope=internal&query=${unit.name}`
          );
          ...
          ebaySearch.setAttribute(
            "data-link",
            `https://www.ebay.com/sch/i.html?_nkw=40k+${unit.name}`
          );
      ...
      let downloadKFON =
        ...
        encodeURIComponent(JSON.stringify(force.force));
      thisList.innerHTML += `
			<hr>
